...
function EditorUI:CreateInputArea(rect, text, toolTip, pattern, font, forceDraw, maxChars, maxLines)
  ...
  data.keys = {
    ...
    Delete = Keys.Delete,
    Enter = Keys.Enter, -- This is technically 13 in Unity but 10 in ASCII?
...
function EditorUI:ResizeInputArea(data, width, height, x, y)
...
function EditorUI:InputAreaKeyCapture(data)

  data.inputTime = data.inputTime + self.timeDelta
  ...
  if(data.inputTime > data.inputDelay) then
    ...
    elseif(Key(data.keys.Delete)) then
      ...
      if(data.onDelete ~= nil) then
        data.onDelete(data)
      end

    elseif(Key(data.keys.Tab)) then
    ...
    elseif(Key(data.keys.Enter)) then
...
function EditorUI:InputAreaOnInput(data, value)
  ...
  -- Need to make sure we are not capturing text when the ctrl key is down
