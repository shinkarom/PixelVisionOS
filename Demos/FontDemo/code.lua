...
local longText = "This is a test to see how long we can make text in a text box?";

local wrapWidth = 12

local White, Yellow, Orange, Grey = 15, 14, 13, 5
...
function Init()
  ...
  DrawText("Font API Demo", 1, 1, DrawMode.Tile, "large", Yellow)
  ...
  DrawTextBox("large Spacing 0", 1, 4, 10, "large", Orange)

  -- Now we can loop through each of the supported characters and display them in the ScreenBufferChip.
  local lines = DrawTextBox(characters, 1, 6, wrapWidth, "large", White, 0)

  DrawTextBox("small Spacing -4", 16, 4, 10, "large", Orange)
  ...
  DrawTextBox(characters, (wrapWidth + 4) * 8, 48, wrapWidth, "small", White, - 4, DrawMode.TilemapCache)
  ...
  DrawText("Long Text - No Wrap", 1, offsetY, DrawMode.Tile, "large", Orange)
  DrawText(longText, 1, offsetY + 2, DrawMode.Tile, "large", White)

  DrawText("Long Text - Wrap", 1, offsetY + 4, DrawMode.Tile, "large", Orange)
  ...
  DrawTextBox(longText, 1, offsetY + 6, 30, "large", White)
  --end
...
function Draw()
  ...
  DrawText("Dynamic Text ".. time, 8, 28 * 8, DrawMode.Sprite, "large", colorOffset)
