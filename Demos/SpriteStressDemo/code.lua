...
local delay = 0
local delayTime = 2
local totalSprites = 500;
local x = 0
local y = 0
local width = 0
local height = 0
local totalColors = 15
local lastFPS = 0
local newFPS = 0
...
function Update(timeDelta)
  ...
  if(delay > delayTime) then
    ...
    BackgroundColor(math.random(1, totalColors))
...
function Draw()
  ...
  for i = 0, totalSprites, 1 do
    ...
    DrawSprite(math.random(375, 410), x, y)
  end
  ...
  DrawText("SPRITES " .. ReadTotalSprites() .. " FPS " .. ReadFPS(), 4, 4, DrawMode.UI, "large", 0)
